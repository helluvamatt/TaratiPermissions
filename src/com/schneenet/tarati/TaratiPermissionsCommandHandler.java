	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		if (COMMAND_TARATIPERMS.equals(command.getName())) {
			if (sender instanceof Player) {
				...
				if (this.plugin.getPermissionManager().has(p,
						COMMAND_TARATIPERMS, p.getWorld().getName())) {
					if (args.length > 0) {
						if (COMMAND_TARATIPERMS_RELOAD.equals(args[0])) {
							if (this.plugin.getPermissionManager().has(
									p,
									COMMAND_TARATIPERMS + "."
											+ COMMAND_TARATIPERMS_RELOAD,
									p.getWorld().getName())) {
								if (args.length > 1) {
									if (COMMAND_TARATIPERMS_RELOAD_ALL
											.equals(args[1])) {
										if (this.plugin
												...
												.has(p,
														COMMAND_TARATIPERMS
																...
																+ COMMAND_TARATIPERMS_RELOAD
																+ "."
																+ COMMAND_TARATIPERMS_RELOAD_ALL,
														p.getWorld().getName())) {
											Player[] players = this.plugin
													.getServer()
													.getOnlinePlayers();
									...
									} else if (COMMAND_TARATIPERMS_RELOAD_USER
											.equals(args[1])) {
										if (args.length > 2) {
											this.plugin
													.processPlayer(this.plugin
															.getServer()
															.getPlayer(args[2]));
										} else {
											...
											p.sendMessage("    /"
													...
													+ COMMAND_TARATIPERMS_RELOAD
													+ " "
													+ COMMAND_TARATIPERMS_RELOAD_USER
													+ " <user>");
										}
									} else {
										p.sendMessage(ChatColor.RED
							...
							sender.sendMessage("    /" + COMMAND_TARATIPERMS + " "
									+ COMMAND_TARATIPERMS_RELOAD);
							sender.sendMessage("    /" + COMMAND_TARATIPERMS + " "
									+ COMMAND_TARATIPERMS_RELOAD + " "
									+ COMMAND_TARATIPERMS_RELOAD_ALL);
							sender.sendMessage("    /" + COMMAND_TARATIPERMS + " "
									+ COMMAND_TARATIPERMS_RELOAD + " "
									+ COMMAND_TARATIPERMS_RELOAD_USER + " <user>");
						}
					} else {
						sender.sendMessage(ChatColor.RED + "Usage:");
						sender.sendMessage("    /" + COMMAND_TARATIPERMS + " "
				...
				if (args.length > 0
						&& COMMAND_TARATIPERMS_RELOAD.equals(args[0])) {
					if (args.length > 1) {
						if (COMMAND_TARATIPERMS_RELOAD_ALL.equals(args[1])) {
							Player[] players = this.plugin.getServer()
									.getOnlinePlayers();
							for (int i = 0; i < players.length; i++) {
								...
								.equals(args[1])) {
							if (args.length > 2) {
								this.plugin.processPlayer(this.plugin
										.getServer().getPlayer(args[2]));
							} else {
								sender.sendMessage(ChatColor.RED
										+ "Not enough arguments.");
								sender.sendMessage(ChatColor.RED + "Usage:");
